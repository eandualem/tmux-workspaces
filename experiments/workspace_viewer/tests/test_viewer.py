...
from viewer import attachment_hosts_viewer, default_data_dir, default_source_socket, launch, parser
...
class LauncherTests(unittest.TestCase):
    ...
    def test_host_recursion_guard_checks_panes_across_linked_and_grouped_sessions(self):
        with tempfile.TemporaryDirectory(prefix="tw-host-", dir="/tmp") as directory:
            socket = str(Path(directory) / "source.sock")
            ...
            try:
                ...
                with patch("viewer.Tmux", side_effect=AssertionError("queried another server")):
