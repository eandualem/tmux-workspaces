...
class OuterClient(Client):
    def __init__(self, socket: str):
        ...
        self.process = subprocess.Popen(
            ...
            stdout=slave,
            stderr=slave,
            env=clean_env() | {"TERM": "xterm-256color", "SHELL": "/bin/sh"},
...
def run(directory: Path) -> None:
    ...
    try:
        source.run(
            "-f", "/dev/null", "new-session", "-d", "-s", "host", "-c", str(directory), "/bin/sh"
        )
        ...
        client = OuterClient(source.socket)
        ...
        def open_viewer():
            ...
            assert "workspaces" in source.run(
        ...
        wait(
            client, lambda: saved(library).tab["id"] == initial.tab["id"], "tab mouse click failed"
        )
        ...
        wait(
            client,
            lambda: (
                "PLUGIN_SOURCE_OK" in source.run("capture-pane", "-p", "-t", "=ordinary-source:")
            ),
        ...
        wait(
            ...
            lambda: (
                ...
                in viewer.run("capture-pane", "-p", "-t", "viewer:.1")
            ),
