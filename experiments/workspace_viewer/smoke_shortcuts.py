...
from controls import direct_sequence
from model import leaves
from smoke import Client, click_button, saved, wait
from smoke_plugin import OuterClient
from terminal import Shells, Tmux
...
def cleanup_library(library: Path) -> None:
    ...
    Tmux(shell_socket).run("kill-server", check=False)
    Path(shell_socket + ".viewer-lock").unlink(missing_ok=True)


def standalone(directory: Path) -> None:
    library = directory / "direct"
    source_socket = socket_path(directory, "absent-source")
    client = Client(["--data-dir", str(library), "--source-socket", source_socket])
    ...
    try:
        ...
        check(
            lambda: (
                "1 Workspaces" in viewer.run("list-panes", "-F", "#{pane_active} #{@viewer_agent}")
            ),
        ...
        for previous_name, new_name, status_line in (
            ...
            assert shell_pid == shells.run(
                "display-message", "-p", "-t", first_terminal, "#{pane_pid}"
            )
        ...
        check(
            lambda: "Workspace options" in sidebar(viewer), "inactive workspace button menu missing"
        )
        button("Rename workspace")
...
def nested(directory: Path) -> None:
    library = directory / "nested"
    outer = Tmux(socket_path(directory, "shortcut-outer"))
    shells = Tmux(socket_path(library, "terminals"))
    client = None
    ...
    try:
        outer.run("-f", "/dev/null", "new-session", "-d", "-s", "host", "/bin/sh")
        ...
        wait(
            client, lambda: "Layouts saved" in sidebar(viewer), "nested sidebar did not initialize"
        )
        original = saved(library)
        terminal = "=" + Shells.name(original.pane) + ":"
        wait(
            client, lambda: shells.run("has-session", "-t", terminal) == "", "nested shell missing"
        )
        ...
        wait(
            client, lambda: len(saved(library).space["tabs"]) == 2, "direct key lost in outer tmux"
        )
        client.pump(0.35)
        ...
        if client is not None:
            client.close()
        outer.run("kill-server", check=False)
        cleanup_library(library)
