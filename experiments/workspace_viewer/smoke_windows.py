...
from terminal import Tmux
from viewer import socket_path, start_demo


def run(directory: Path, terminfo: str | None) -> None:
    source_socket = socket_path(directory, "window-smoke-source")
    source = Tmux(source_socket)
    ...
    def open_window(terminal_env=None):
        client = Client(args, terminal_env=terminal_env)
    ...
    try:
        ...
        wait(
            second, lambda: "1 researcher" in selected(second_view), "second window selection wrong"
        )
        ...
        print(
            "PASS: concurrent windows, independent selection and input, shared workspace/tab edits,"
        )
        print(
            "      terminal close/reopen, old lock ignored, "
            "other windows and source sessions survive."
        )
        if terminfo:
        ...
        server.shutdown()
        server.server_close()
        source.run("kill-server", check=False)
