...
class GhosttyShellClient(Client):
    def __init__(self, library: Path, source_socket: str):
        ...
        # Ghostty uses login argv[0] only on macOS. Linux uses a portable exec.
        wrapper = (
        ...
        super().__init__(
            ...
            launcher=wrapper,
            cwd=library.parent,
            terminal_env=env | {"HOME": str(library.parent)},
        )
...
def _exercise(resources: FixtureResources) -> None:
    ...
    library = resources.library("launch library 'quoted'")
    source_socket = socket_path(directory, "absent-source")
    shells = Tmux(socket_path(library, "terminals"))
    client = resources.own_client(GhosttyShellClient(library, source_socket))
    ...
    wait(
        client,
        lambda: "LAUNCH_RETAINED:retained:/tmp" in shells.run("capture-pane", "-p", "-t", terminal),
