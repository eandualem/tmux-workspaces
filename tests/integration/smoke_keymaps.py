...
from tests.integration.smoke_shortcuts import cleanup_library, sidebar
...
def launch(library: Path, directory: Path, arguments: list[str]):
    client = Client(
        [
            "--data-dir",
            str(library),
...
def standalone(directory: Path) -> None:
    library = directory / "custom"
    config = directory / "keys.toml"
    config.write_text(CONFIG)
    client, viewer, shells = launch(library, directory, ["--keymap", str(config)])
    second = None
    defaults = None
    try:
        ...
        new_tab_help = next(
            line.strip() for line in original_help.splitlines() if "New tab" in line
        )
        ...
        client.type("\x01u")
        wait(
            client, lambda: len(saved(library).space["tabs"]) == 3, "running keymap was not frozen"
        )

        # Another viewer takes the new snapshot without changing the first one.
        other_library = directory / "second"
        second, other_viewer, other_shells = launch(
            other_library, directory, ["--keymap", str(config)]
        )
        second.type("\x01y")
        wait(
            second, lambda: len(saved(other_library).space["tabs"]) == 2, "new snapshot not loaded"
        )
        ...
        defaults, default_viewer, _ = launch(library, directory, ["--no-keymap"])
        ...
        for item in (client, second, defaults):
            if item is not None:
                item.close()
        cleanup_library(library)
        cleanup_library(directory / "second")
...
def nested(directory: Path) -> None:
    ...
    config.write_text(CONFIG)
    outer = Tmux(str(directory / "outer.sock"))
    client = None
    try:
