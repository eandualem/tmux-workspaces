...
class StandaloneClient(Client):
    ...
    def __init__(self, arguments: list[str], terminal_env: dict[str, str]):
        super().__init__(
            ...
            launcher=[str(Path(__file__).resolve().parents[2] / "run")],
        )
...
def exercise(resources: FixtureResources) -> None:
    ...
    source = resources.server("standalone-source")
    alternate = resources.server("standalone-alternate")
    ...
    def open_window(tmux: Tmux):
        client = resources.own_client(
            StandaloneClient(
                ["--data-dir", str(library), "--source-socket", tmux.socket],
                {"HOME": str(directory)} | env,
            )
        )
